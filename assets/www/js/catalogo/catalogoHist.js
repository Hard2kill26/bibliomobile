$('#catalogohist').live('pageshow', function(event) {
	var id = getUrlVars()["id"];
	var user = getUrlVars()["user"];
	$.getJSON(serviceURL + 'getcatalogo2.php?id='+ id + '&user=' + user , displaycatalogohist);
	
	$("#reservar2").click(function(event){
...
function displaycatalogohist(data) {
	var user = getUrlVars()["user"];
	var item = data.items;
	$('#catalogoList2 li').remove();
	
	$('#catalogoList2').append('<li id="tam"> ' + item.nombre_pub + '</li>');
	
	if(item.tipo==1)
		{	
			$('#info2').append('<li data-role="list-divider">Informaci&oacute;n</li>'
			...
			$('#disponibilidad2').append('<li data-role="list-divider">Disponibilidad</li>' 
		...
		$('#reservar2').append('<input type="button" data-theme="e" data-icon="plus" value="Reservar"/></div>');

		$('#catalogoList2').listview('refresh');
		$('#info2').listview('refresh');
		$('#disponibilidad2').listview('refresh');
		$("[type='button']").button();
}
